...
// byte.
always @(negedge dec_clk) begin: WITH_UART_M_DEC_0_HEX_COUNTER
    integer digit;
    if (reset == 1) begin
        m_dec_0_q_int_l[0] <= 0;
        m_dec_0_q_int_l[1] <= 0;
...
always @(negedge dec_clk) begin: WITH_UART_M_DEC_0_BIN_COUNTER
    if (reset == 1) begin
...
always @(posedge uart_0_baud) begin: WITH_UART_UART_0_CHECKFORENDBITS
    if (reset == 1) begin
...
always @(posedge clk) begin: WITH_UART_UART_0_DELAY_DRDY
    if (reset == 1) begin
        uart_0_drdy1 <= 1;
        uart_0_drdy2 <= 1;
    end
...
always @(posedge uart_0_drdy_int) begin: WITH_UART_UART_0_LATCH_ON_READY
    if (reset == 1) begin
        rx_data <= 00;
    end
    else begin
